...
- **AI · realistic** — a fully local pipeline: your prompt is translated if needed
  (opus-mt, CPU), painted into a reference image (SD-Turbo), cut out (rembg), and
  sculpted into a real mesh (TripoSR) on your GPU. A prompt like `çeşme` becomes an
  actual tiered fountain with ~100k triangles in a few seconds.
...
Measured on an RTX 5060 Laptop (8GB): generation is a 3–6s burst at ~100% GPU /
~45W / 64°C peak, ~6.1GB VRAM resident, idle between requests. Laptop-friendly.
